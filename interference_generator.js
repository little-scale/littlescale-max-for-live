...
// Normalized setters
function lambda_norm(v) {
    lambda = map(v, 0, 1, 0.0004, 0.0007); // 400–700 nm
}

function slitsep_norm(v) {
    slit_sep = map(v, 0, 1, 0.05, 0.3); // mm
}

function slitwidth_norm(v) {
    slit_width = map(v, 0, 1, 0.005, 0.05); // mm
}

function set_gamma(v) {
    gamma = clamp(v, 0.1, 3.0); // perceptual contrast
}
